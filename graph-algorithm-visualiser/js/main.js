...
var visitedColour = "#4CAF50"; //Colour used to indicate a node/edge is visited.
var unvisitedColour = "#FF8A80"; //Colour used to indicate a node/edge is unvisited.
var checkColour = "#5C6BC0"; //Colour used when a node/edge is checked.
var delay = 45; //Millisecond delay for algorithms.
...
var graphArea = {
  start: function() {
    this.canvas = $("#graph-view")[0]; //Gets the canvas object.
    this.context = this.canvas.getContext("2d"); //Gets the canvas' 2D context.
    this.context.lineWidth = 2; //Graph edge thickness.
    this.nodeSet = []; //Set of all nodes in the graph.
    this.nodeSize = 10; //Size in pixels of the node.
    this.generateRandomNodes(); //Generates random nodes for the graph.
  },
  clear: function() { //Clears the canvas.
    this.context.clearRect(0, 0, this.canvas.width, this.canvas.height);
  },
  drawAll: function(colour) { //Draws all nodes in the graph.
    for (var i = 0; i < this.nodeSet.length; i++) {
      this.nodeSet[i].update(colour);
    }
  },
  resetStats: function() { //Resets graph statistics.
    graphStats.setValue("");
    graphStats.setName("-");
  },
  generateRandomNodes: function() { //Creates random nodes in the graph.
    this.clear();
    this.nodeSet = [];
    this.nodeCount = parseInt($("#select-node-count").val());
    for (var i = 0; i < this.nodeCount; i++) { //Assigns a random position to the node.
...
var graphStats = {
  setValue: function(value) { //Sets the value of the statistic.
    $("#stat-value").html(value);
  },
  incrementValue: function(value) { //Increments the statistic by a specific value.
    $("#stat-value").html(parseInt($("#stat-value").html()) + value);
  },
  setNodeCount: function(value) { //Sets the value of the node count.
    $("#node-count").html(value);
  },
  setName: function(name) { //Sets the name of the statistic being recorded.
    $("#stat-name").html(name);
  }
};
...
function runAlgorithm(algorithm) {
  if (graphArea.available) {
    algorithm(); //Runs the specified algorithm.
    window.location = "#top-view"; //Places the canvas in view for the user.
  }
}
...
//Draws the edge and nodes connected by the edge onto the canvas.
...
function prims() {
  ...
  var unvisitedNodes = []; //Set of unvisited nodes.
  var visitedNodes = []; //Set of visited nodes.
  var edgeCount = 0; //Number of edges considered in the MST.

  for (var k = 0; k < graphArea.nodeCount; k++) {
    unvisitedNodes[k] = graphArea.nodeSet[k]; //Adds all nodes to the visited set.
  }
  ...
  while (unvisitedNodes.length > 0) {
    ...
    for (var i = 0; i < visitedNodes.length; i++) {
      for (var j = 0; j < unvisitedNodes.length; j++) {
        ...
        //Checks if the node has a smaller distance.
...
function kruskals() {
  ...
  var disjointSet = []; //Disjoint set for checking if nodes are connected.
  var edgeCount = 0; //Number of edges considered in the MST.

  //Adds all nodes to the disjoint set as unconnected.
  for (var k = 0; k < graphArea.nodeCount; k++) {
  ...
  var edgeSet = []; //Set of all edge in the graph.
  ...
  //Loops through every sorted edge.
